...
from src.prompts import (BASELINE_PROMPT_WIKI, PLAN_VERIFICATION_TWO_STEP_PROMPT_WIKI, 
                     ...
                     FINAL_VERIFIED_TWO_STEP_PROMPT_WIKI_CATEGORY, PLAN_AND_EXECUTION_JOINT_PROMPT_WIKI_CATEGORY,
                     FINAL_VERIFIED_JOINT_PROMPT_WIKI_CATEGORY)

class ChainofVerification():
    def __init__(self, model_id, top_p, temperature, task, setting, questions, access_token):
    ...
    def run_verification_chain(self, original_question):
        if self.task == "wikidata":
            ...
            if self.setting == "two_step":
                ...
                final_verified_tokens = self.generate_response(final_verified_prompt, 300)
                ...
                plan_and_execution_tokens = self.generate_response(plan_and_execution_prompt, 500).split("\n\n")[1]
                ...
                final_verified_prompt = f"""<s>[INST] <<SYS>>{final_verified_prompt_tmpl}\n<</SYS>>\nFinal Refined Answer: [/INST]"""
                final_verified_tokens = self.generate_response(final_verified_prompt, 150).split("\n\n")[1]
            ...
            if self.setting == "two_step":
                ...
                plan_and_execution_prompt_tmpl = PLAN_AND_EXECUTION_JOINT_PROMPT_WIKI_CATEGORY.format(original_question=original_question, baseline_response=baseline_tokens)
                plan_and_execution_prompt = f"""<s>[INST] <<SYS>>{plan_and_execution_prompt_tmpl}\n<</SYS>>\nVerification Questions and Answers: [/INST]"""
                plan_and_execution_tokens = self.generate_response(plan_and_execution_prompt, 400).split("\n\n")[1]
                
                final_verified_prompt_tmpl = FINAL_VERIFIED_JOINT_PROMPT_WIKI_CATEGORY.format(original_question=original_question, baseline_response=baseline_tokens, verification_questions_and_answers=plan_and_execution_tokens)
                final_verified_prompt = f"""<s>[INST] <<SYS>>{final_verified_prompt_tmpl}\n<</SYS>>\nFinal Refined Answer: [/INST]"""
                final_verified_tokens = self.generate_response(final_verified_prompt, 100).split("\n\n")[1]
            ...
            if self.setting == "two_step":
                ...
                plan_verification_tokens = self.generate_response(plan_verification_prompt, 400).split("\n\n")[1]
                ...
                execute_verification_tokens = self.generate_response(execute_verification_prompt, 400).split("\n\n")[1]
                ...
                final_verified_prompt_tmpl = FINAL_VERIFIED_TWO_STEP_PROMPT_MULTI_QA.format(original_question=original_question, baseline_response=baseline_tokens, verification_questions=plan_verification_tokens, verification_answers=execute_verification_tokens)
                final_verified_prompt = f"""<s>[INST] <<SYS>>{final_verified_prompt_tmpl}\n<</SYS>>\nFinal Refined Answer: [/INST]"""
                final_verified_tokens = self.generate_response(final_verified_prompt, 300).split("\n\n")[1]
    ...
    def run_and_store_results(self):
        ...
        if (self.model_id, self.task, self.setting) in valid_combinations:
            result_file_path = f'./results/{self.model_id}_{self.task}_{self.setting}_results.json'
