...
## TODO Shirin: This file and the one for factored are very similar. They should be merged with each other.
## TODO Shirin: Remove extra prints!

class ExecuteVerificationChainTwoStep(Chain):
    ...
    def _call(
        ...
        execution_prompt_self_llm = PromptTemplate.from_template(prompts.EXECUTE_PLAN_PROMPT_SELF_LLM_TWO_STEP)

        # Executing the verification questions, either using search tool or self llm
        # for question in verification_questions_list:
        execution_prompt_value = execution_prompt_self_llm.format_prompt(**{"verification_questions": verification_questions_str})
        verification_answers_llm_result = self.llm.generate_prompt([execution_prompt_value], callbacks=run_manager.get_child() if run_manager else None)
        verification_answers_str = verification_answers_llm_result.generations[0][0].text
        verification_answers_list = verification_answers_str.split("\n")
        print(f"verification_answer_str: {verification_answers_str}")
